                    ...
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end items-center gap-1.5">
                                ...
                                <form method="POST" action="{{ route('installments.destroy', $installment) }}" class="inline-block delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete(this)" class="p-2 text-red-600 bg-red-50 hover:bg-red-100 hover:text-red-900 rounded-lg transition duration-150" title="{{ __('app.cancel') }}">
...
function confirmDelete(button) {
    const message = '{{ __("app.confirm_delete") }}';
    if (confirm(message)) {
