...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    ...
    <div class="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">{{ __('app.edit_category') }}</h1>
            <p class="text-sm text-gray-500 mt-1">Modify details for category "{{ $category->name }}".</p>
        </div>
    ...
    <!-- Form Card -->
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="bg-white p-8 rounded-xl shadow-sm border border-gray-100">
        @csrf
        @method('PUT')

        <div class="mb-6">
            <label class="block text-gray-700 text-sm font-medium mb-2">{{ __('app.category_name') }} <span class="text-red-500">*</span></label>
            <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="w-full border px-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-150 {{ $errors->has('name') ? 'border-red-500' : 'border-gray-300' }}" placeholder="e.g., Laptop, Accessories">
            @error('name')
        ...
        <div class="mb-6">
            ...
            <input type="text" name="brand" value="{{ old('brand', $category->brand) }}" class="w-full border px-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-150 {{ $errors->has('brand') ? 'border-red-500' : 'border-gray-300' }}" placeholder="e.g., Dell, HP, Logitech">
            @error('brand')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
