...
@section('content')
<h1 class="text-2xl font-bold mb-4">Record Sale (Stock Out)</h1>
<form method="POST" action="{{ route('admin.sales.store') }}" class="bg-white p-6 rounded shadow">
    @csrf
    <div class="mb-4"><label>Customer name</label><input name="customer_name" class="w-full border px-2 py-1"></div>
    ...
    <div id="items">
        <div class="item mb-3">
            ...
            <input type="number" step="0.01" name="items[0][price]" class="w-1/6 border px-2 py-1 inline-block">
        </div>
    </div>
...
document.getElementById('addItem').addEventListener('click', ()=>{
    ...
    div.innerHTML = `
        ...
        `@foreach($products as $p) <option value="{{ $p->id }}">{{ $p->name }} ({{ $p->stock }})</option> @endforeach` +
        `</select>
        <input type="number" name="items[${idx}][quantity]" value="1" min="1" class="w-1/6 border px-2 py-1 inline-block">
        <input type="number" step="0.01" name="items[${idx}][price]" class="w-1/6 border px-2 py-1 inline-block">
    `;
