...
<div class="content max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b border-slate-200/80">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.sales.index') }}"
               class="w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-amber-600 hover:border-amber-200 hover:bg-amber-50/50 flex items-center justify-center shadow-xs transition duration-150"
               title="{{ __('app.back') ?? 'ត្រឡប់ក្រោយ' }}">
                <i class="fas fa-arrow-left text-sm"></i>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2.5">
                    <span class="w-9 h-9 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-lg shadow-sm border border-amber-100">
                        <i class="fas fa-edit"></i>
                    </span>
                    {{ __('app.edit_direct_sale') }}
                </h1>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.sales.show', $sale) }}"
               class="inline-flex items-center gap-2 px-4 py-2 text-xs sm:text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm hover:bg-slate-50 hover:text-slate-900 transition" style="text-decoration: none;">
                <i class="fas fa-receipt text-slate-400"></i> {{ __('app.view_receipt') }}
            </a>
            <a href="{{ route('admin.sales.index') }}"
               class="inline-flex items-center gap-2 px-4 py-2 text-xs sm:text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm hover:bg-slate-50 hover:text-slate-900 transition" style="text-decoration: none;">
    ...
    <form method="POST" action="{{ route('admin.sales.update', $sale) }}" id="saleForm">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 sm:p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="relative customer-select-container">
                            ...
                            <input type="hidden" name="customer_id" id="customerId" value="{{ old('customer_id', $sale->customer_id) }}">
                            <div class="relative">
                                <input type="text" id="customerSelectSearch"
                                       value="{{ old('customer_name', $sale->customer?->name ?? $sale->customer_name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="customer_name" id="customerName" value="{{ old('customer_name', $sale->customer_name) }}"
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden sticky top-6">
                    ...
                    <div class="bg-slate-900 px-5 py-4 text-white flex items-center justify-between">
                        <h3 class="text-sm font-bold flex items-center gap-2 tracking-wide">
                            <i class="fas fa-receipt text-amber-400"></i> {{ __('app.receipt') }}
                        </h3>
                        <span class="text-[11px] font-mono bg-slate-800 text-amber-300 px-2 py-0.5 rounded border border-slate-700">
                            {{ $sale->invoice_no }}
                        </span>
                    </div>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="date" name="sale_date" value="{{ old('sale_date', $sale->sale_date ? \Carbon\Carbon::parse($sale->sale_date)->format('Y-m-d') : date('Y-m-d')) }}"
                            ...
                            <div>
                                ...
                                <select name="payment_method"
                                    ...
                                    @foreach($paymentMethods as $pm)
                                        <option value="{{ $pm->name }}" {{ old('payment_method', $sale->payment_method) == $pm->name ? 'selected' : '' }}>
                        ...
                        <div class="border-t border-dashed border-slate-200 pt-3 space-y-2.5 text-xs sm:text-sm">
                            <div class="flex items-center justify-between text-slate-600">
                                ...
                                <div class="text-right font-semibold text-slate-900">
                                    <span id="subtotalLabel" class="block">${{ number_format($sale->subtotal ?? 0, 2) }}</span>
                                    <span id="subtotalLabelRiel" class="block text-[11px] text-slate-400 font-normal">0 ៛</span>
                                </div>
                            </div>

                            <div class="flex items-center justify-between text-slate-600" id="taxRow" style="{{ ($sale->tax_amount ?? 0) > 0 ? '' : 'display: none;' }}">
                                <span class="font-medium">{{ __('app.tax') }} <span id="taxRateDisplay" class="text-slate-400 text-xs"></span></span>
                                <div class="text-right font-semibold text-slate-900">
                                    <span id="taxLabel" class="block">${{ number_format($sale->tax_amount ?? 0, 2) }}</span>
                        ...
                        <div class="bg-gradient-to-br from-blue-600 to-indigo-700 text-white rounded-xl p-4 shadow-sm space-y-1">
                            ...
                            <div class="text-right">
                                <div id="grandTotal" class="text-2xl sm:text-3xl font-black tracking-tight font-mono">${{ number_format($sale->total ?? 0, 2) }}</div>
                        ...
                        <div>
                            ...
                            <textarea name="note" rows="2"
                                      ...
                                      placeholder="...">{{ old('note', $sale->note) }}</textarea>
                        </div>

                        {{-- Submit Button --}}
                        <button type="submit"
                                class="w-full py-3 px-4 bg-amber-600 hover:bg-amber-700 active:bg-amber-800 text-white font-bold text-sm rounded-xl shadow-sm hover:shadow transition-all flex items-center justify-center gap-2 cursor-pointer">
                            <i class="fas fa-save text-base"></i> {{ __('app.save') }}
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    ...
    const initialItems = {!! json_encode($sale->items->map(function($it) {
        return [
            ...
            'tax_type' => $it->product?->tax_type ?? 'exclusive'
        ];
    })) !!};
    ...
    function itemRow(index, initial = null) {
        ...
        const prodId = initial ? initial.product_id : '';
        const prodName = initial ? initial.product_name : '';
        const qty = initial ? initial.quantity : 1;
        const price = initial ? parseFloat(initial.price).toFixed(2) : '';
        const sub = initial ? (qty * initial.price).toFixed(2) : '0.00';
        const taxable = initial ? (initial.is_taxable ? '1' : '0') : '0';
        const taxRate = initial ? initial.tax_rate : '0';
        const taxType = initial ? initial.tax_type : 'exclusive';
        ...
        div.innerHTML = `
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                <div class="sm:col-span-6 relative product-select-container">
                    ...
                    <input type="hidden" name="items[${index}][product_id]" class="item-product-id" value="${prodId}" data-taxable="${taxable}" data-tax-rate="${taxRate}" data-tax-type="${taxType}" required>
                    <div class="relative">
                        <input type="text" 
                               value="${escapeHtml(prodName)}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="number" name="items[${index}][quantity]" value="${qty}" min="1" required
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="number" step="0.01" min="0" name="items[${index}][price]" value="${price}" required
    ...
    function addItem(initial = null) {
        document.getElementById('items').appendChild(itemRow(idx, initial));
        idx++;
        updateNumbers();
        updateRemoveButtons();
        calculateTotal();
